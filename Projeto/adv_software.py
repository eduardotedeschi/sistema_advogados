...
class Funcs():
    ...
    def gerenciar_clientes(self):
        ...
        if(self.nome_busca_entry.winfo_exists):
            self.nome_busca_entry.place_forget() 
            self.nome_busca_entry.place(in_=self.frame_cliente, relx= 0.66, rely=0.44, relwidth=0.30)
        else:
    ...
    # Função para criar os widgets da tela de gerar documento e afins
    def gerar_doc(self):
        ...
        if(self.nome_busca_entry.winfo_exists):
            self.nome_busca_entry.place_forget() 
            self.nome_busca_entry.place(in_=self.frame_gerar_doc, relx= 0.66, rely=0.44, relwidth=0.30)
        else:
            self.nome_busca_entry = ttk.Entry(self.frame_gerar_doc, style="Big.TEntry")
            self.nome_busca_entry.place(relx= 0.66, rely=0.44, relwidth=0.30)
            self.nome_busca_entry.bind("<KeyRelease>", lambda event: self.busca_cliente())
        ...
        # Criação botão deletar registro
        self.bt_del_registro = ttk.Button(self.frame_gerar_doc, text="Deletar Registro", style='Accent.TButton')
        self.bt_del_registro.place(relx= 0.02, rely=0.75, relwidth=0.17)

        # Criação botão exportar word
        self.bt_export_word = ttk.Button(self.frame_gerar_doc, text="Exportar Word", style='Accent.TButton')
        self.bt_export_word.place(relx= 0.60, rely=0.75, relwidth=0.17)

        # Criação botão exportar pdf
        self.bt_export_pdf = ttk.Button(self.frame_gerar_doc, text="Exportar PDF", style='Accent.TButton')
        self.bt_export_pdf.place(relx= 0.80, rely=0.75, relwidth=0.17)
...
class App(Funcs):
    ...
    def carregar_db(self):
        # Adicionando carregamento dos dataframes
